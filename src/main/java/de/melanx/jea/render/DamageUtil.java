...
public class DamageUtil {
    ...
    public static void draw(MatrixStack matrixStack, IRenderTypeBuffer buffer, Minecraft mc, DamagePredicate predicate, EntityPredicate.AndPredicate targetEntity, boolean sourceIsPlayer, boolean targetIsPlayer) {
        ...
        Pair<EntityInfo, EntityInfo> entities = getInfo(predicate, sourceIsPlayer, targetIsPlayer, list::add);
        EntityInfo source = entities.getLeft();
        EntityInfo target = entities.getRight();
        ...
        matrixStack.push();
        matrixStack.translate(RECIPE_WIDTH - 40, yValue - 2, 0);
        ...
        matrixStack.pop();
        matrixStack.push();
        matrixStack.translate(40, yValue - 2, 0);
        JeaRender.normalize(matrixStack);
    ...
    public static void addTooltip(List<ITextComponent> tooltip, Minecraft mc, DamagePredicate predicate, EntityPredicate.AndPredicate targetEntity, boolean sourceIsPlayer, boolean targetIsPlayer, double mouseX, double mouseY) {
        ...
        float entityScale = (yValue - SPACE_TOP - 4) / 32f;
        source.addTooltip(tooltip, mc, LootUtil.asLootPredicate(predicate.sourceEntity), RECIPE_WIDTH - 40, yValue - 2, JeaRender.normalScale(entityScale), mouseX, mouseY);
        target.addTooltip(tooltip, mc, targetEntity, 40, yValue - 2, JeaRender.normalScale(entityScale), mouseX, mouseY);
    }
    ...
    private static Pair<EntityInfo, EntityInfo> getInfo(DamagePredicate predicate, boolean sourceIsPlayer, boolean targetIsPlayer, Consumer<ITextComponent> lines) {
        ...
        if (predicate.type.isProjectile != null) {
            ...
            if (predicate.type.isProjectile) {
                if (source.type == null) {
                    ...
                    source.held = new ItemStack(Items.BOW);
    ...
    private static class EntityInfo {
        ...
        public void render(MatrixStack matrixStack, IRenderTypeBuffer buffer, Minecraft mc, @Nullable EntityPredicate.AndPredicate entity, EntityTransformation transformation) {
            if (this.type != null && this.type == EntityType.PLAYER) {
                ...
                DefaultEntityProperties properties = new DefaultEntityProperties(this.type, true, this.fire, false, this.held, this.swing + 0.5f, 0, this.usingStack ? 1 : 0, this.hurt ? 10 : 0, 0);
        ...
        public void addTooltip(List<ITextComponent> tooltip, Minecraft mc, @Nullable EntityPredicate.AndPredicate entity, double absoluteX, double absoluteY, double totalScale, double mouseX, double mouseY) {
            if (this.type != EntityType.PLAYER) {
                DefaultEntityProperties properties = new DefaultEntityProperties(this.type, true, this.fire, false, this.held, this.swing, 0, this.usingStack ? this.held.getUseDuration() : 0, this.hurt ? 10 : 0, 0);
